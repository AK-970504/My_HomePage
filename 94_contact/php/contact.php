...
$textMessage .= "お名前: " . ($_POST['name_01'] ?? "") . "\n";
$textMessage .= "フリガナ: " . ($_POST['name_02'] ?? "") . "\n";
$textMessage .= "メールアドレス: " . ($_POST['email'] ?? "") . "\n";
$textMessage .= "お問い合わせ内容:" . ($_POST['message'] ?? "") . "\n";
// 画像ファイルの添付処理は別途必要
// ヘッダー情報（返信先など）
...
if(mail($to, $subject, $message, $headers)) {
	echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>送信完了</title></head><body>';
	echo '<script>alert("送信が完了しました");';
	echo 'window.location.href = "/94_contact/contact.html";</script>';
	echo '</body></html>';
	exit;
} else {
